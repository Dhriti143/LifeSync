...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [journalRes, quoteRes] = await Promise.all([
        getJournals(0, 4),
        getTodayQuote(),
      ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        {/* ── HERO ── */}
        <section className="relative mb-8 overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 p-6 shadow-2xl sm:mb-10 sm:p-10 md:p-14">
          ...
          <div className="relative z-10 flex flex-col gap-10 xl:flex-row xl:items-center xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="max-w-2xl text-base leading-relaxed text-white/80 sm:text-lg">
                Reflect on your thoughts, track your moods, and build habits — every day.
              </p>
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                >
                  <BookOpen size={18} /> View Journals
                </Link>
              </div>
            </div>
            ...
            <div className="w-full xl:max-w-sm">
              <div className="rounded-3xl border border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
                <p className="mb-4 text-xs uppercase tracking-[0.2em] text-white/60">Quote of the Day</p>
                {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          {/* LEFT COL — Habits + Recent Journals */}
          <div className="space-y-6 lg:col-span-2">
            ...
            <section>
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-gray-900">Recent Journals</h2>
                  <p className="mt-0.5 text-sm text-gray-500">Your latest reflections and memories.</p>
                </div>
                <Link
                  to="/journals"
                  className="inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 transition hover:gap-2.5"
                >
              ...
              {loading ? (
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="animate-pulse rounded-3xl border bg-white p-6">
                      <div className="mb-4 h-5 w-1/2 rounded bg-gray-200" />
                      <div className="mb-2 h-4 rounded bg-gray-100" />
                      <div className="mb-6 h-4 w-4/6 rounded bg-gray-100" />
                    </div>
                  ))}
                </div>
              ) : journals.length === 0 ? (
                <div className="rounded-[2rem] border border-dashed border-gray-300 bg-white p-10 text-center sm:p-16">
                  <h3 className="mb-3 text-xl font-bold text-gray-900">No journal entries yet</h3>
                  <button
                    onClick={() => setIsModalOpen(true)}
                    className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 font-semibold text-white transition hover:bg-indigo-700"
                  >
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  {journals.map((journal) => (
                    ...
                    >
                      ...
                      <h3 className="mb-2 line-clamp-1 text-xl font-bold text-gray-900">{journal.title}</h3>
                      <p className="mb-5 line-clamp-3 text-sm leading-relaxed text-gray-500">{journal.content}</p>
      ...
      <JournalViewModal
        isOpen={isViewOpen}
        onClose={() => { setViewJournal(null); setIsViewOpen(false); }}
        journal={viewJournal}
      />
    </div>
  );
